...
async function run() {
  try {
    ...
    const collageCollection = client.db("clgboolingdb").collection("collage");
...
app.get('/', (req, res) => {
  res.send('Collage booking is running')
})


app.listen(port, () => {
  console.log(`Collage Booking is running on port ${port}`);
})